...
namespace amqpCommon
{
    ...
    void deleteQueue(AMQP::Channel &channel, const std::string &queue_name, bool ifUnused, bool ifEmpty) {
        channel.removeQueue(queue_name,
    ...
    void deleteExchange(AMQP::Channel &channel, const std::string &exchange_name, bool ifUnused) {
        channel.removeExchange(exchange_name,
                               ifUnused ? AMQP::ifunused : AMQP::direct
                )
                .onSuccess([]() {
                    std::cout << "Exchange deleted\n";
                })
                .onError([](const char* error) {
    ...
    void
    sendMessage(AMQP::Channel &channel, std::string_view exchange1, std::string_view routing_key, std::string_view body,
                bool persistent) {
/*AMQP::Table props;
        props.setDeliveryMode(persistent ? 2 : 1);
        props.setContentType("text/plain");
        props.setHeader("custom-header", "value");*/

        channel.publish(exchange1, routing_key, body);
    }

    void declareQueue(AMQP::Channel &channel, std::string_view queue1, std::string_view exchange1) {
        channel.declareQueue(queue1)
                .onSuccess([&queue1]() { std::cout <<fmt::format("Queue [{}]declared\n",queue1);  })
                .onError([](const char* msg) { std::cerr << "Queue error: " << msg << "\n"; });
    ...
    void declareExchange(AMQP::Channel &channel, std::string_view exchange1) {
        channel.declareExchange(exchange1, AMQP::direct)
                .onSuccess([&exchange1]() { std::cout << fmt::format("Exchange [{}]declared\n",exchange1); })
                .onError([](const char* msg) { std::cerr << "Exchange error: " << msg << "\n"; });
    }
}